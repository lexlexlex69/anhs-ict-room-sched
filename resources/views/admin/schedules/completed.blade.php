        ...
        @include(' _message')
            ...
            <div class="flex justify-between items-center">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="inline-flex rounded-md shadow-sm" role="group">
                        <a href="{{ url('admin/schedules/completed?filter=all' . (request('per_page') ? '&per_page='.request('per_page') : '')) }}" 
                        ...
                        <a href="{{ url('admin/schedules/completed?filter=week' . (request('per_page') ? '&per_page='.request('per_page') : '')) }}" 
                           class="px-4 py-2 text-sm font-medium {{ request('filter') === 'week' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100' }}">
                            This Week
                        </a>
                        <a href="{{ url('admin/schedules/completed?filter=month' . (request('per_page') ? '&per_page='.request('per_page') : '')) }}" 
                    ...
                    <div class="flex items-center">
                        ...
                        <select id="per-page-selector" class="border border-gray-300 rounded px-2 py-1 text-sm">
            ...
            @if(request('filter') === 'week')
                <div class="mt-2 text-sm text-gray-600">
                    <p>Viewing completed schedules from this week: {{ \Carbon\Carbon::now()->startOfWeek()->format('M d') }} - {{ \Carbon\Carbon::now()->endOfWeek()->format('M d, Y') }}</p>
                </div>
            @elseif(request('filter') === 'month')
                <div class="mt-2 text-sm text-gray-600">
                    <p>Viewing completed schedules from this month: {{ \Carbon\Carbon::now()->startOfMonth()->format('M d') }} - {{ \Carbon\Carbon::now()->endOfMonth()->format('M d, Y') }}</p>
                </div>
            @endif
